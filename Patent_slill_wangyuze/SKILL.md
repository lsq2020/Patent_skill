...
description: 对小分子、靶点、适应症及其组合开展可复核的医药专利检索、专利族归并、权利要求/结构/申请人/时间线抽取、法律状态核验、技术路线、FTO 风险和创新空间分析，并分别生成抽取报告、专利族地图报告、技术路线图报告、风险与 FTO 报告、创新空间假设报告、证据链报告和来源目录报告，同时生成 WIPO 风格的专利景观图、交互式/静态数据可视化、结构化 Schema 输出、证据/不确定性/失败案例记录和可复现实验。用户要求“查某个分子/靶点/适应症的专利”“做专利地图”“看技术路线”“找空白点”“评估专利风险”或需要中美/全球医药专利分析时使用。不得将本 Skill 的结果表述为法律意见、侵权结论或可直接用于申报/诉讼的结论。
---
...
把每次运行当作一个可复核的函数：固定输入范围，输出报告、`case-output.json` 和日志。机器可读字段与接口见 [references/output-schema.json](references/output-schema.json)。

- 每个关键事实保留来源 URL、文献/claim/事件定位、日期、证据等级和置信度；
- 每个不确定结论写明影响、反例和下一步；每个标准失败模式写明是否发生、触发条件、影响和 fallback；
- 保存输入/输出哈希、运行命令和 Skill 版本；必要时使用 `scripts/run_reproducibility.py` 重放本地报告流水线；
- 用 `scripts/validate_case.py`、`scripts/validate_output_schema.py` 和 `scripts/run_reproducibility.py` 验证工程链条，不把内部推断写成科学或法律结论。
...
当用户要求“把每个可检索来源都检索一遍”时，不只检查首页：先运行 `scripts/audit_public_sources.py` 对目录中的去重 URL 做访问和入口发现，再用案例目录中的 `source-search-portals.json` 声明可复用的 GET/POST/浏览器检索协议，运行 `scripts/search_public_sources.py --project-dir <case-dir>`。执行台账必须覆盖所有公开页面记录，并明确区分：

- `executed`：已实际提交只读查询并保存 HTTP/最终 URL/页面标题/响应摘要哈希/结果信号；
- `browser_manual`：已确认公开检索入口，但查询依赖 JavaScript、浏览器会话、验证码或状态表单；
- `not_mapped`：页面公开，但当前没有可靠的专利检索端点，不能猜测参数。

查询词至少覆盖分子通用名、开发代号/商品名、靶点别名和适应症别名。`result_count` 只能是页面启发式数字信号，必须标成“信号”而不是正式命中总数；逐条专利、权利要求和族成员仍要回写 `source-log.jsonl` 与证据链。
...
当用户要求“类似样例的 FTO 报告”或需要正式文档交付时，运行 `scripts/build_fto_docx.py --project-dir <case-dir>`，从 FTO 输入、检索计划、专利族、claim 要素、证据链和候选排序数据生成模板化 A4 DOCX。报告采用“封面—概览（申请人/受让人相关性矩阵）—检索范围—技术方案—技术特征—专利族初筛—逐件相关专利与对比结论—权利要求特征比对表—证据链—附录—免责声明”的结构；含表格、横向宽表、可点击来源链接和页眉页脚。该脚本是样例结构的可复用实现，不复制样例数据，也不把候选排序升级为法律结论。
...
静态结构使用 Mermaid 或 Markdown 图表；需要筛选、点击、钻取时生成独立 HTML；需要汇报或归档时同时生成 PPT/PNG 或 PDF 版本。图中每个关键节点必须绑定 `family_id` 或 `finding_id`，并能够回到专利号、claim/事件位置和来源链接。可视化字段参见 [references/landscape-visualization.md](references/landscape-visualization.md)。

### FTO 风格统计可视化

统一使用 `scripts/build_report_visuals.py --project-dir <case-dir>` 生成依赖外部图库的 SVG/HTML 资产。默认图表包括：专利族技术主题、最早优先权年度、申请人/受让人、法域覆盖、权利要求类别、状态信号、FTO 复核优先级、证据置信度、证据类型、来源角色、访问层级、来源检索状态、实际执行状态、响应信号和检索轮次。图形不局限于柱状图：比例构成使用环形图，年度变化使用折线图，法域×族使用热力矩阵，优先权×FTO 初筛分数使用散点/气泡图，族与技术阶段关系使用网络图。每张图都写入 `visuals/manifest.json`，记录图形类型、数据来源字段、统计定义和数值。
...
├── case-output.json                 # 机器可读输出 Schema（需要时）
├── reproducibility-report.json      # 本地重放记录（需要时）
├── reproducibility-runs/            # 本地重放快照（需要时）
...
├── public-source-search-audit.csv  # 逐来源公开访问/检索状态（需要时）
├── public-source-search-audit.json
├── public-source-search-report.md
├── source-portal-overrides.json    # 旧入口到新版检索页的映射（需要时）
├── source-search-portals.json      # 公开来源的 GET/POST/浏览器检索协议（需要时）
├── public-source-search-results.csv # 逐来源实际检索执行台账（需要时）
├── public-source-search-results.json
├── public-source-search-results-report.md
...
- `build_case_output.py`：把案例输入、族/claim/证据/FTO 记录、报告路径、不确定性、失败案例和输入/输出哈希组装为稳定的 `case-output.json`。
- `validate_output_schema.py`：不依赖第三方包检查 `case-output.json` 的顶层字段、记录数组、不确定性、失败案例和复现字段。
- `run_reproducibility.py`：不重新访问外部网站，按参数重放本地报告流水线并记录返回码和规范化输出哈希。
...
- `audit_public_sources.py`：对来源目录中的每个去重 URL 做只读访问审计；跟踪 HTTP 跳转并提取页面推荐的专利检索入口，对已知的公开检索端点执行查询；将登录、验证码、订阅、JavaScript/POST 表单和失效链接标为人工或待复核，不绕过访问控制。对于用户在可见浏览器中确认可访问、但机器请求被反爬拦截的入口，单独标记为 `public_searchable_browser_verified`，不能混写成自动请求成功。
- `search_public_sources.py`：按 `source-search-portals.json` 对公开来源实际提交统一查询；对稳定 GET/安全 POST 端点执行读取，对浏览器专属入口记录 `browser_manual`，对公开但未发现可靠端点的页面记录 `not_mapped`，并生成执行报告、统计图数据和可复核响应摘要。不得绕过登录、订阅、验证码或隐藏访问控制。
...
- 是否真正执行了所有已映射公开入口，并将 `executed`、`browser_manual`、`not_mapped` 与登录/验证码/订阅/阻断来源分开；没有把搜索页加载、页面数字或 CAPTCHA 响应误写成命中文献数。
...
- 是否生成 `case-output.json`，并通过 Schema、字段交叉计数、不确定性、失败案例和输入/输出哈希校验；是否保存最小复现实验报告。

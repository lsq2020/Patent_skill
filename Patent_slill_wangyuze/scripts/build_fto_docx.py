...
def owner_overview_rows(families, candidates):
    """Create the reference-style applicant/assignee relevance matrix."""
...
def comparison_conclusion(ranking, family):
    """Use evidence-bounded wording for the reference-style per-patent conclusion."""
    priority = ranking.get("review_priority") or "LOW"
    matched = ranking.get("matched_features") or "—"
    partial = ranking.get("partial_features") or "—"
    if priority == "HIGH":
        return (
            f"该候选族同时出现拟实施对象/适应症与核心作用机制信号（完整命中记录：{matched}；部分命中记录：{partial}）。"
            "应进入首批完整 claim chart 和目标法域状态核验队列；本段仅说明初筛关联，不确认权利要求覆盖或侵权。"
        )
    if priority == "MEDIUM":
        return (
            f"该候选族对研究对象/适应症及 PD-L1 阻断存在较强初筛关联（完整命中记录：{matched}；部分命中记录：{partial}）。"
            "当前尚未证明肺部监测、生化检测或结肠炎处置等必要特征被同一有效独立权利要求组合限定，建议先核验国家阶段、独立权利要求、分案/继续申请和审查历史。"
        )
    return (
        f"该候选族当前表现为部分或邻近技术关联（完整命中记录：{matched}；部分命中记录：{partial}）。"
        "现有证据不足以构成拟实施方案的实施障碍；仍应检查同族分支、目标法域法律状态及权利要求变化。"
    )
...
def build_report(project):
    ...
    add_heading(doc, "概览")
    add_heading(doc, "申请人/受让人相关性概览", level=2)
    add_table(
        doc,
        ["当前专利权人", "高相关", "中相关", "低相关"],
        owner_overview_rows(families, candidates),
        [2200, 2000, 2800, 1300],
        body_size=8.2,
    )
    add_para(doc, "相关度分组沿用本案例的复核优先级；它是排序和资源分配标签，不是侵权概率、有效性判断或法律意见。", size=8.5, color=MUTED, italic=True, before=4, after=8)
    ...
    add_heading(doc, "逐件相关专利与对比结论", level=2)
    for row in candidates:
        family = family_map.get(row.get("family_id"), {})
        add_para(doc, f"相关专利：{row.get('representative_document') or row.get('family_id')}", size=10, color=NAVY, bold=True, before=4, after=3)
        metadata = [[
            row.get("representative_document") or row.get("family_id"),
            row.get("review_priority") or "LOW",
            compact(family.get("official_status") or "未核验", 150),
            compact(row.get("source_url") or family.get("source_url") or "—", 170),
        ]]
        add_table(doc, ["相关专利", "相关度", "当前状态信号", "来源"], metadata, [2800, 1700, 2500, 6950], body_size=7.8)
        add_para(doc, "对比结论：" + comparison_conclusion(row, family), size=8.8, color=INK, after=7)
    comparison_rows = []

...
def risk_relation(coverage):
    """Map evidence wording to a reference-style FTO comparison label."""
    if coverage in ("明确披露", "explicit", "explicitly_disclosed"):
        return "证据明确披露", "重点复核"
    if coverage in ("可能覆盖", "possible", "potential"):
        return "可能关联，需完整 claim chart", "待核验"
    if coverage in ("未见披露", "not_seen"):
        return "当前记录未见直接披露", "当前记录可排除"
    return "记录不足，不能判断", "待核验"


def owner_overview(families, ranking):
    family_map = {row.get("family_id"): row for row in families}
    grouped = {}
    for row in ranking:
        family = family_map.get(row.get("family_id"), {})
        owner = family.get("applicant_or_assignee") or "未归一化申请人/受让人"
        bucket = row.get("review_priority") or "LOW"
        grouped.setdefault(owner, {"HIGH": [], "MEDIUM": [], "LOW": []})
        if bucket not in grouped[owner]:
            bucket = "LOW"
        grouped[owner][bucket].append(row.get("representative_document") or row.get("family_id"))
    rows = []
    for owner, buckets in sorted(grouped.items(), key=lambda item: item[0]):
        cells = []
        for bucket in ("HIGH", "MEDIUM", "LOW"):
            docs = buckets[bucket]
            cells.append(f"{len(docs)} 项候选：{', '.join(docs)}" if docs else "暂无")
        rows.append([owner, *cells])
    return rows
...
def render_risk(case, scope, identity, families, claims, ranking, plan, catalog, files):
    ...
    lines = [common_header("风险与 FTO 报告", case, scope, identity, catalog, files["generated"]), "## 1. 概览", "", "本报告按照“概览—专利初筛—逐件相关专利—权利要求特征比对—检索附录”的 FTO 工作流组织。高/中/低仅表示复核顺序，不代表侵权风险等级。", "", table(["当前申请人/受让人", "高相关", "中相关", "低相关"], owner_overview(families, ranking) or [["—", "—", "—", "—"]]), "", "## 2. 风险边界", "", "本报告识别的是值得继续核验的重叠信号和 FTO 工作队列，不是侵权、不侵权、有效性或自由实施法律意见。排序分数只代表复核优先级。", "", "## 3. 拟实施方案与特征分级", "", plan.get("technical_solution", "未记录"), ""]
    feature_rows = []
    ...
    lines += ["", "## 5. 逐件相关专利与权利要求特征比对", "", "以下结构对应参考 FTO 报告中的“相关专利—相关度—对比结论—权利要求特征表”。本案例的 claim-elements 是结构化抽取记录，不是完整官方独立权利要求文本，因此“对比特征”和“覆盖状态”均为初筛证据标签，不能替代法律等同判断。", ""]
    for row in sorted(ranking, key=priority_sort):
        family_id = row.get("family_id")
        family = family_map.get(family_id, {})
        document = family.get("representative_document") or row.get("representative_document") or "—"
        priority = row.get("review_priority", "—")
        lines.append(f"### 相关专利：{document} · {family_id}")
        lines.append("")
        lines.append(table(["相关专利", "相关度", "当前状态信号", "来源"], [[
            source_link(row, document), priority, compact(row.get("official_status_signal") or family.get("official_status"), 280), row.get("status_source") or family.get("status_source") or "未记录",
        ]]))
        lines.append("")
        if priority == "HIGH":
            conclusion = "当前记录同时命中拟实施对象、机制/用途和核心技术特征，进入重点 claim chart 队列；尚不能据此确认落入有效权利要求。"
        elif priority == "MEDIUM":
            conclusion = "当前记录对拟实施对象/适应症和 PD-L1 阻断形成中等强度初筛信号，部分监测与处置特征未建立直接 claim linkage；应优先核验完整独立权利要求和目标法域状态。"
        else:
            conclusion = "当前记录主要是部分命中或邻近技术信号，尚不足以作为实施障碍结论；保留用于边界复核、同族追踪和创新空间分析。"
        lines.append("#### 对比结论")
        lines.append("")
        lines.append(conclusion)
        lines.append("")
        comparison_rows = []
        family_claims = claim_map.get(family_id, [])
        if not family_claims:
            family_claims = [{"claim_location": "未建立逐条 claim 记录", "element": "—", "claim_category": family.get("claim_categories", "—"), "coverage": "待核验", "confidence": "—", "notes": "需要下载目标法域官方文本并建立 claim chart。"}]
        for claim in family_claims:
            relation, status = risk_relation(claim.get("coverage"))
            matched = row.get("matched_features") or "无"
            partial = row.get("partial_features") or "无"
            comparison = f"完整命中：{matched}；部分命中：{partial}"
            reason = claim.get("notes") or "当前记录来自结构化 claim 抽取；需核对完整独立权利要求、法域成员和法律事件。"
            if claim.get("coverage") == "明确披露":
                reason = f"当前 claim 记录标记为明确披露；{reason}"
            elif claim.get("coverage") == "可能覆盖":
                reason = f"当前 claim 记录标记为可能覆盖；{reason}"
            comparison_rows.append([
                claim.get("claim_location") or "—",
                compact(claim.get("element"), 300),
                compact(comparison, 180),
                relation,
                compact(reason, 300),
                status,
            ])
        lines.append(table(["权要编号/定位", "权利要求特征", "对比特征", "初筛关系", "判定理由", "权利要求覆盖状态"], comparison_rows))
        lines.append("")
        lines.append(f"**下一步**：下载目标法域官方文本，核对完整独立权利要求、分案/继续申请、审查档案、年费/异议/无效和拟实施方案的逐项要素。代表文献：{source_link(row, document)}。")
        lines.append("")
    lines += ["## 6. 风险雷达", "", table(["风险层", "触发条件", "当前判断", "必须补证据"], [
    ...
    ]), "", "## 7. 检索附录", "", "### 7.1 技术特征、关键词与分类号", ""]
    search_feature_rows = []
    for feature in plan.get("features", []):
        clusters = []
        for cluster in plan.get("keyword_expansion", []):
            if cluster.get("id") in feature.get("keyword_clusters", []):
                clusters.append(f"{cluster.get('label')}: {', '.join(cluster.get('expanded_terms', []))}")
        search_feature_rows.append([feature.get("text"), "；".join(clusters) or "—", ", ".join(feature.get("classifications", [])) or "—"])
    lines.append(table(["技术特征描述", "关键词拓展", "分类号"], search_feature_rows or [["—", "—", "—"]]))
    lines += ["", "### 7.2 检索式与执行边界", ""]
    search_rows = []
    for search_round in plan.get("search_rounds", []):
        search_rows.append([search_round.get("id"), search_round.get("title"), search_round.get("formula"), search_round.get("status")])
    lines.append(table(["编号", "检索轮次", "检索式", "执行状态"], search_rows or [["—"] * 4]))
    lines += ["", "## 8. FTO 结论边界", "", "当前数据不足以确认自由实施或侵权。若要进入商业决策，应先对高优先族逐项制作 claim chart，并由目标法域专利律师复核法律状态和解释问题。聚合网站、摘要命中和初筛分数不能替代 CNIPA、USPTO、WIPO、EPO 或其他目标法域官方登记簿与完整权利要求文本。"]
    return "\n".join(lines) + "\n"
...
def render_source_catalog(case, scope, identity, catalog, files):
    ...
    audit = files.get("source_audit") or {}
    public_search = files.get("public_search") or {}
    if audit:
        counts = audit.get("counts_by_access_class", {})
        lines += ["", "## 4. 公开访问与检索审计", "", f"本轮对目录中的 **{audit.get('source_count', '—')} 个去重 URL** 执行只读访问检查；**{audit.get('public_page_count', '—')} 个**返回公开页面信号，**{audit.get('direct_search_count', '—')} 个**确认存在可用公开检索入口，其中自动请求成功 **{audit.get('automated_search_count', '—')} 个**，另有浏览器/用户可见页面确认的入口。", "", table(["审计产物", "入口"], [["逐来源 CSV", link("public-source-search-audit.csv", "public-source-search-audit.csv")], ["机器可读 JSON", link("public-source-search-audit.json", "public-source-search-audit.json")], ["审计报告", link("public-source-search-report.md", "public-source-search-report.md")], ["入口覆盖配置", link("source-portal-overrides.json", "source-portal-overrides.json")]]), "", table(["访问层级", "数量"], [[key, value] for key, value in sorted(counts.items())] or [["—", "—"]])]
    if public_search:
        search_counts = public_search.get("counts_by_attempt_status", {})
        lines += ["", "## 5. 公开来源实际检索执行", "", f"本轮将公开页面和检索入口纳入 **{public_search.get('record_count', '—')} 条**执行台账，其中已实际提交只读查询 **{search_counts.get('executed', 0)} 条**，浏览器人工入口 **{search_counts.get('browser_manual', 0)} 条**，尚未建立稳定端点 **{search_counts.get('not_mapped', 0)} 条**。实际提交不等于已完成逐条文献抽取。", "", table(["执行产物", "入口"], [["逐来源执行 CSV", link("public-source-search-results.csv", "public-source-search-results.csv")], ["机器可读 JSON", link("public-source-search-results.json", "public-source-search-results.json")], ["执行报告", link("public-source-search-results-report.md", "public-source-search-results-report.md")], ["检索入口配置", link("source-search-portals.json", "source-search-portals.json")]]), "", table(["执行状态", "数量"], [[key, value] for key, value in sorted(search_counts.items())] or [["—", "—"]])]
    lines += ["", visual_block(files, ["source-kind", "source-access-class", "source-search-status", "source-execution-status", "source-result-signal", "search-round"])]
    lines += ["", "## 5. 完整来源清单", ""]
    rows = []
    ...
    lines += ["", "## 6. 访问限制", "", "对需登录/验证码/订阅、无法检全库、仅显示著录项、没有全文或链接失效的来源，写入 `pending/manual`，不要填充虚假的结果数量；法律状态仍以目标法域官方登记簿为准。"]
    return "\n".join(lines) + "\n"
...
def build_reports(project):
    ...
    source_audit = load_json(project / "public-source-search-audit.json", {})
    ...
    index_lines = [f"# {project.name} 模块化报告索引", "", f"> 生成时间：{generated} · 结构化数据目录：`{project}`", "", "## FTO 风格统计入口", "", "- [打开交互式统计总览](report-visuals.html)", "- [查看图表数据清单](visuals/manifest.json)", "- [公开来源访问与检索审计](public-source-search-report.md)", "- [公开来源实际检索执行](public-source-search-results-report.md)", "", "## 报告清单", ""]
    for filename, title, _ in REPORTS:
        index_lines.append(f"- [{title}]({filename})")
    index_lines += ["", "## 输入与过程数据", "", "- `research_scope.json` / `identity.json`：研究范围和实体消歧", "- `*-patent-families.csv`：族级数据", "- `*-claim-elements.csv`：权利要求要素", "- `*-evidence.csv`：证据链", "- `fto-search-plan.json`：FTO 特征、检索轮次和来源目录", "- `fto-candidate-ranking.csv`：候选排序", "- `source-log.jsonl`：实际访问日志", "- `source-portal-overrides.json`：新版检索入口/浏览器确认入口", "- `source-search-portals.json`：本案例公开来源检索协议", "- `public-source-search-audit.*`：来源访问与公开检索审计", "- `public-source-search-results.*`：实际检索执行台账", "- `case-output.json`：机器可读 Schema、指标、不确定性和失败案例", "- `reproducibility-report.json` / `reproducibility-runs/`：本地重放记录和快照", "- `visuals/`：依赖无外部图库的 SVG 统计图和 manifest", "", "## 总体限制", "", "报告将未核验的国家阶段、聚合状态、缺失 claim 和未采集结构明确标出；不把模块报告升级为法律意见。"]
    ...
    state["reports"]["source_audit"] = "complete" if source_audit else "partial"
    state["reports"]["public_source_search"] = "complete" if public_search else "partial"
    state["reports_generated_at"] = generated
    if build_case_output:
        build_case_output(project)
        state["reports"]["machine_output"] = "complete"
        state["reports"]["output_contract"] = "complete"

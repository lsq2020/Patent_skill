...
class ReportingIntegrationTests(unittest.TestCase):
    def test_modular_report_build_includes_knowledge_graph(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn('src="knowledge-graph.html?embed=report&amp;view=technology&amp;depth=2"', report_index)
            self.assertIn('class="graph-embed__hint"', report_index)
            self.assertIn("拖拽节点 · 滚轮缩放 · 点击查看证据", report_index)
            self.assertNotIn("在报告内直接拖拽节点、滚轮缩放并沿 family / claim / finding / source 回溯证据。", report_index)

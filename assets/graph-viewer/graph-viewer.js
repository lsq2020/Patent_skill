(() => {
  ...
  const compactOrganizationLabel = (value) => {
    const text = String(value || "")
      .trim()
      .replace(/^C\/o\s+/i, "")
      .replace(/\s+(?:AG|AB|Inc\.?|Ltd\.?|LLC|PLC|Corp\.?|Corporation|Company|Co\.?)$/i, "");
    return compactLabel(text, 18);
  };
  const nodeDisplayLabel = (node) => {
    const properties = node.properties || {};
    if (node.type === "patent_family") {
      const document = properties.representative_document || properties.representative_application || "";
      return [node.label, document].filter(Boolean).join("\n");
    }
    if (node.type === "claim") return compactLabel(properties.claim_category || node.label.split(" · ")[0], 18);
    if (node.type === "applicant") return compactOrganizationLabel(node.label);
    if (node.type === "causal_concept") return compactLabel(node.label, 28);
    if (node.type === "source") return compactLabel(node.label, 24);
    return node.label;
  };
  const nodeBaseSizes = {
    research_object: 40,
    ...
    style: [
      {
        ...
        style: {
          ...
          "font-family": "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Inter, 'Noto Sans SC', 'PingFang SC', 'Microsoft YaHei', sans-serif",
          "font-size": 9,
          "font-weight": 500,
          "line-height": 1.22,
      ...
      { selector: 'node.editorial-node[type = "research_object"]', style: { "background-color": "#2b78e4", "border-color": "#a9caf7", "border-width": 1.5, width: 40, height: 40, color: "#173f91", "font-size": 11.5, "font-weight": 600, "text-max-width": 150 } },
      ...
      { selector: "edge.edge-active", style: { label: "data(label)", width: 1.8, opacity: 0.96, color: "#25354d", "font-size": 8, "font-weight": 600, "target-arrow-shape": "triangle", "text-rotation": "autorotate", "text-background-color": "#ffffff", "text-background-opacity": 0.92, "text-background-padding": 2, "text-margin-y": -7, "z-index": 8 } },
      ...
      { selector: "node:selected", style: { "border-color": "#1261d6", "border-width": 2.4, "underlay-color": "#2b78e4", "underlay-opacity": 0.13, "underlay-padding": 9, "z-index": 999, "z-index-compare": "manual" } },
      { selector: "edge:selected", style: { width: 2.2, opacity: 1, "overlay-color": "#2b78e4", "overlay-opacity": 0.06 } },
  ...
  function scheduleSemanticZoom() {
    if (semanticZoomFrame) return;
    semanticZoomFrame = window.requestAnimationFrame(applySemanticZoom);
  }
